...
# Every cached helper takes the dataset key as its first argument, so two datasets can never
# share a cache entry and switching is just a different key.


def current_dataset() -> str:
    f = st.session_state.get("filters")
    return (f or {}).get("dataset", DS.DEFAULT_DATASET)


@st.cache_resource(show_spinner="Loading SCADA data, well tests and running the power-method pipeline...")
def get_results(dataset: str = DS.DEFAULT_DATASET) -> Results:
    """The whole pipeline for one dataset, computed once per server process."""
    return run_pipeline(dataset)


@st.cache_resource
def get_last_tests(dataset: str = DS.DEFAULT_DATASET) -> pd.DataFrame:
    r = get_results(dataset)
...
def well_slice(dataset: str, well: str, start: str, end: str) -> pd.DataFrame:
    """All rows of one well inside [start, end] (end date inclusive)."""
    return vr.slice_period(get_results(dataset).rt, well, _ts(start), _ts(end))


@st.cache_data(show_spinner=False, max_entries=64)
def rate_series(dataset: str, well: str, start: str, end: str, freq: str, steady_only: bool) -> pd.DataFrame:
    ...
    d = well_slice(dataset, well, start, end)
    if freq == "30min":
...
@st.cache_data(show_spinner=False, max_entries=64)
def signal_series(dataset: str, well: str, start: str, end: str, freq: str) -> pd.DataFrame:
...
def period_stats(dataset: str, well: str, start: str, end: str, k_mode: str, steady_only: bool,
                 wc_correction: bool = False) -> dict:
    return vr.period_stats(well_slice(dataset, well, start, end), k_mode, steady_only, wc_correction)
...
@st.cache_resource
def mape_by_well(dataset: str = DS.DEFAULT_DATASET) -> pd.DataFrame:
    """Per-well MAPE/MdAPE indexed by (well, method)."""
    return get_results(dataset).mape.set_index(["scope", "method"])


@st.cache_resource
def gas_by_well(dataset: str = DS.DEFAULT_DATASET) -> pd.DataFrame:
    """Per-well intake-vs-bubble-point summary, indexed by well. Empty without a bubble point."""
    g = get_results(dataset).gas
    return g.set_index("WELL_NAME") if len(g) else g


@st.cache_data(show_spinner=False, max_entries=16)
def pvt_series(dataset: str, well: str) -> pd.DataFrame:
    """Per-test water cut and B_liq for the water-cut chart."""
    t = get_results(dataset).test_pvt
    if not len(t):
        return t
...
@st.cache_data(show_spinner=False, max_entries=64)
def window_events(dataset: str, wells: tuple[str, ...], start: str, end: str) -> pd.DataFrame:
    return events_in_window(get_results(dataset).events, wells, _ts(start), _ts(end))


@st.cache_data(show_spinner=False, max_entries=64)
def shading(dataset: str, well: str) -> dict:
    ...
    return dict(gaps=vr.gap_intervals(r.events, well),
                ...
                regimes=regime_marks(dataset, well))
...
def tests_in_range(dataset: str, well: str, start: str, end: str) -> pd.DataFrame:
    r = get_results(dataset)
