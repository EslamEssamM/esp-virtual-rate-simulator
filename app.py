...
from ui.sidebar import pending_scope, render_sidebar  # noqa: E402
...
pages = [
    ...
    st.Page("app_pages/rules.py", title="Filter rules", icon=":material/rule_settings:", url_path="rules"),
    st.Page("app_pages/methodology.py", title="Methodology", icon=":material/functions:", url_path="methodology"),
    st.Page("app_pages/export.py", title="Export to Excel", icon=":material/table_view:", url_path="export"),
]
...
get_results(pending_scope())   # first call runs the pipeline (with a spinner); later calls are instant
render_sidebar()
page.run()
